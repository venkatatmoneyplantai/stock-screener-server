...
| Screening result ranking/scoring methodology | Decided, differs per round | Round 1: strict pass/fail gate, every technical rule must pass. Round 2: **not** strict — its 6 fundamental rules split into two independent 3-rule "buckets" (EPS, Operating Profit), each needing 2-of-3; a symbol passes if either bucket clears its own gate (`evaluateGrowthBucket` in `fundamental-rules.ts`). Added the Operating Profit bucket so a stock with strong operating performance but noisy EPS (e.g. one-off tax/interest items) isn't excluded — see `_docs/architecture/rounds.md`. The full `/screening/results` endpoint uses neither — it's a simple passed/total score, not a gate |
